...
args
  ...
  .option('tags', 'Module tags')
  .option('location', 'Where would you like to put the module')
  .option('test', 'generate index.js and test.js files')
...
module.exports = function (opts) {
  if (opts.prompts) {
    modulePromptsIntial = modulePromptsInitial.concat(opts.prompts);
  }
...
function startPrompts(data, opts) {
  ...
  if(modulePromptsInitial.length > 0) {
    nygAction(modulePromptsInitial, opts, globs);
  }
  else {
    var prompts = [];
    if(flags.tags || flags.description || flags.stability) {
      if(!flags.tags) prompts.push(promptTags);
      if(!flags.description) prompts.push(promptDescription);
      if(!flags.stability) prompts.push(promptStability);
    }
    nygAction(prompts, opts, globs);
  }
}
...
function setConfigFromFlags(gen) {
  if(flags.name || flags.description || flags.location || flags.tags || flags.stability) {
    ...
    if(flags.stability) gen.config.set('stability', flags.stability);
  }
}

function nygAction(prompts, opts, globs) {
  var isPostPublish = opts.isPostPublish;
  var gen = nyg(prompts, globs)
    ...
    .on('postcopy', function () {
      if (isPostPublish) {
        opts.postCopyCallback && opts.postCopyCallback(opts.depsData.localImports, configs.outputDir);
      }
    })
...
function makeModuleDirectory(gen, done) {
  fs.mkdirp(configs.outputDir, function () {
...
function setDetailPrompts(modulePromptsDetails) {
  if(flags.description || flags.tags || flags.stability) {
    modulePromptsDetails = [];
    if(!flags.description) modulePromptsDetails.push(promptDescription);
    if(!flags.tags) modulePromptDetails.push(promptTags);
    if(!flags.stability) modulePromptDetails.push(promptStability);
    return modulePromptsDetails;
  }
  else return modulePromptsDetails;
}
