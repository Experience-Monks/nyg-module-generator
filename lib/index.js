...
function startPrompts(data, opts) {
  ...
  var gen = nyg(modulePrompts, globs)
    ...
    .on('postinstall', function () {
      if (isPostPublish) {
        opts.moduleDeps && opts.moduleDeps.forEach(function (dep, i) {
          var cmd = 'npm i ' + dep + ' --save';
          var child = spawn(cmd, {cwd: configs.outputDir});
          child.on('exit', (err) => {
            (i == opts.moduleDeps.length - 1) && publishModule(gen, cb);
          })
        })
      } else {
  ...
  child.on('exit', function (err) {
    if (err === 0) {
      ...
      gen.prompt({
        type: "confirm",
        name: "npmPublish",
        message: "Would you like to publish this mudule on npm?",
        default: false
      }, function (data) {
